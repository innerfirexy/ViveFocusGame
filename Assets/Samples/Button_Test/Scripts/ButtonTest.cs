...
public class ButtonTest : MonoBehaviour {
    ...
    private Color unpressed_color;

    public GameObject Button_Touch = null;
    private Text touch_text = null;
    public GameObject Button_Trigger = null;
    private Text trigger_text = null;
    public GameObject Button_Menu = null;
    private Text menu_text = null;
    ...
    {
        SceneManager.LoadScene (1); // Load Evac scene
    }
	...
	void Start ()
    {
        ...
        if (this.Button_Trigger != null)
        {
            this.trigger_text = this.Button_Trigger.GetComponent<Text>();
            PrintDebugLog("Start() Get Text of " + this.Button_Trigger.name);
        }
        if (this.Button_Menu != null)
        {
            this.menu_text = this.Button_Menu.GetComponent<Text>();
            PrintDebugLog("Start() Get Text of " + this.Button_Menu.name);
        }
        pressed_color = new Color(0.384f, 0.967f, 0.114f);
        unpressed_color = new Color(0.749f, 0.2f, 0.2f);
    }
    ...
    //new updateTouchText
    private void updateTouchTextNew()
    {
        if (this.touch_text == null)
            return;
        if (WaveVR_Controller.Input(this.DeviceType).GetPressDown(WVR_InputId.WVR_InputId_Alias1_Touchpad))
        {
            this.touch_text.text = "已按下";
            this.touch_text.color = pressed_color;
        }
        if (WaveVR_Controller.Input(this.DeviceType).GetPressUp(WVR_InputId.WVR_InputId_Alias1_Touchpad))
        {
            this.touch_text.text = "未按下";
            this.touch_text.color = unpressed_color;
        }
    }

    private void updateTrigger()
    {
        if (this.trigger_text == null)
        {
            return;
        }
        if (WaveVR_Controller.Input(this.DeviceType).GetPressDown(WVR_InputId.WVR_InputId_Alias1_Trigger))
        {
            this.trigger_text.text = "已按下";
            this.trigger_text.color = pressed_color;
        }
        if (WaveVR_Controller.Input(this.DeviceType).GetPressUp(WVR_InputId.WVR_InputId_Alias1_Trigger))
        {
            this.trigger_text.text = "未按下";
            this.trigger_text.color = unpressed_color;
        }
    }
    ...
    private void updateMenu()
    {
        ...
        {
            this.menu_text.text = "已按下";
            this.menu_text.color = pressed_color;
        }
        if (WaveVR_Controller.Input(this.DeviceType).GetPressUp(WVR_InputId.WVR_InputId_Alias1_Menu))
        {
            this.menu_text.text = "未按下";
            this.menu_text.color = unpressed_color;
        }
    }
